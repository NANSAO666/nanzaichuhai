<template>
    <view class="home-page">
        ...
        <view class="hero-section">
            ...
            <view class="yacht-icon">
                <text class="yacht-emoji">⛵</text>
            </view>
        </view>

        <!-- 快速入口 -->
        <view class="quick-entry">
            <view class="entry-card" @click="goToClubs">
                <text class="entry-icon">🏝️</text>
                <text class="entry-title">游艇码头</text>
                <text class="entry-desc">五大码头任选</text>
            </view>
            <view class="entry-card" @click="goToBooking">
                <text class="entry-icon">📅</text>
                <text class="entry-title">立即预约</text>
                <text class="entry-desc">快速预订游艇</text>
            </view>
            <view class="entry-card" @click="goToService">
                <text class="entry-icon">💬</text>
                <text class="entry-title">客户服务</text>
                <text class="entry-desc">AI+人工客服</text>
            </view>
        </view>
        ...
        <view class="section">
            <view class="section-header">
                <text class="section-title">精选码头</text>
                <text class="section-more" @click="goToClubs">查看全部 →</text>
            </view>
            <scroll-view scroll-x class="club-scroll">
                <view class="club-card" v-for="club in activeClubs.slice(0, 3)" :key="club.id" @click="goToDetail(club.id)">
                    <view class="club-img">
                        <text v-if="!club.images || club.images.length === 0" class="club-emoji">🚤</text>
                        <image v-else :src="club.images[0].base64" mode="aspectFill" class="club-image"/>
                    </view>
                    <view class="club-info">
                        <text class="club-name">{{ club.name }}</text>
                        <text class="club-area">{{ club.area }}</text>
                        <text class="club-price">¥{{ club.pricing[0]?.price }}起/3小时</text>
                    </view>
                </view>
            </scroll-view>
        </view>
            ...
            <view class="about-card">
                ...
                <view class="contact-row">
                    <view class="contact-item" @click="callPhone">
                        ...
                        <text class="contact-text">{{ brand.phone || '13003192344' }}</text>
                    </view>
                    <view class="contact-item" @click="copyWechat">
                        <text class="contact-icon">💬</text>
                        <text class="contact-text">微信: {{ brand.wechat || 'DaogoG' }}</text>
                    </view>
                </view>
            </view>
        </view>
...
export default {
    ...
    methods: {
        async loadData() {
            try {
                ...
                if (res.brand) {
                    ...
                    storage.set('brand', res.brand)
                }
                if (res.clubs) {
                    this.clubs = res.clubs
                    storage.set('clubs', res.clubs)
                }
            } catch (e) {
        ...
        goToClubs() {
            uni.switchTab({ url: '/pages/clubs/clubs' })
        },
        goToBooking() {
...
.club-card {
    ...
    margin-right: 20rpx;
    overflow: hidden;
    box-shadow: 0 4rpx 16rpx rgba(0, 74, 124, 0.08);
}
