    ...
    <view class="mine-page">
        ...
        <view class="user-section">
            ...
            <view class="user-info">
                <view class="user-name">我的订单</view>
                <view class="user-desc">南崽出海 · 赴一场江海浪漫</view>
            </view>
        </view>
        ...
        <view class="order-tabs">
            <view 
                :class="['order-tab', currentTab === 'all' ? 'active' : '']"
                @click="currentTab = 'all'"
            >全部</view>
            <view 
                :class="['order-tab', currentTab === 'pending' ? 'active' : '']"
                @click="currentTab = 'pending'"
            >待确认</view>
            <view 
                :class="['order-tab', currentTab === 'done' ? 'active' : '']"
                @click="currentTab = 'done'"
            >已确认</view>
            <view 
                :class="['order-tab', currentTab === 'cancelled' ? 'active' : '']"
        ...
        <view class="order-list" v-if="filteredOrders.length > 0">
            <view class="order-card" v-for="order in filteredOrders" :key="order.orderId">
                ...
                <view class="order-body">
                    ...
                    <view class="order-row">
                        <text class="order-label">船型</text>
                        <text class="order-value">{{ order.boatSize }}</text>
                    </view>
                    <view class="order-row">
                        <text class="order-label">出海日期</text>
                        <text class="order-value">{{ order.bookingDate }} {{ order.timeSlot }}</text>
                    </view>
                    <view class="order-row">
                        <text class="order-label">出行人数</text>
                        <text class="order-value">{{ order.pax }}人</text>
                    </view>
                    <view class="order-row" v-if="order.addons && order.addons.length">
                        <text class="order-label">增值服务</text>
                        <text class="order-value">{{ order.addons.join('、') }}</text>
                    </view>
                </view>
        ...
        <view class="empty-state" v-else>
            ...
            <view class="empty-btn" @click="goToClubs">去预约游艇</view>
        </view>
    </view>
</template>
    ...
    methods: {
        async loadOrders() {
            try {
                ...
                this.orders = res.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
...
.order-tabs {
    ...
    position: relative;
    z-index: 10;
    box-shadow: 0 4rpx 16rpx rgba(0, 74, 124, 0.1);
}
